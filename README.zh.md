...
按 `cowork-mdm` 运行的位置选安装通道。

| 运行在哪里 | 安装方式 |
|---|---|
| 管理员工作站（编写配置用） | `brew install krislavten/tap/cowork-mdm` |
| 员工 Mac（Wave 2 Script payload 用） | 从 [Releases](https://github.com/krislavten/cowork-mdm/releases) 下载签名 + 公证的 `.pkg` 安装包，通过你 MDM 的 package-deployment 机制下发。安装到 `/opt/cowork-mdm/bin/` 并 symlink 到 `/usr/local/bin/cowork-mdm`。未配置 Developer ID secrets 前 Release 里的 pkg 是 unsigned（见[维护者说明](#%E5%90%AF%E7%94%A8%E7%AD%BE%E5%90%8D--%E5%85%AC%E8%AF%81%E7%9A%84-pkg)），unsigned 对开发 / CI 可用，但大多数 MDM 集群会拒绝它下发到集群。 |
| CI / 临时机器 | Release 页的 `tar.gz`，解压即用。 |

```bash
# 在管理员工作站 —— 企业 gateway 部署的 6 步标准路径：
...
发版由 tag 触发。推送 `v*` tag，`.github/workflows/release.yml` 先跑 GoReleaser，然后由一个 macOS 后续任务构建 `.pkg` 安装包。

发版任务同时推到三处：

1. **GitHub Releases** 本仓库 —— tar.gz + zip 压缩包 + `.pkg` 安装包 (arm64 + x86_64) + `checksums.txt`。用默认的 `GITHUB_TOKEN`。
2. **Homebrew tap** `krislavten/homebrew-tap` —— 需要本仓库配置 `HOMEBREW_TAP_GITHUB_TOKEN` secret，PAT 对 tap 仓库需要 **contents:write**。没配置时只有 brew formula 推送会失败，其他照常。
3. **macOS `.pkg` 安装包** —— 默认是 unsigned。只要配齐 Apple Developer ID 的 secrets（见下），下次发版会自动签名 + 公证。Unsigned pkg 适合开发 / CI 测试，以及有能力用企业内部证书重新签名的组织；但 Jamf Pro / Kandji / Intune 等主流 MDM 要求 pkg 带 Developer ID Installer 签名（或企业内部已信任证书）才会下发到集群。

一次性配置 Homebrew secret：
...
对 release workflow 本身而言签名是可选的 —— 不配 secrets 也能跑，只是产出 unsigned pkg —— 但对 MDM 下发是硬要求。七个 secret 全配齐后，每次推 `v*` tag 都会产出用 Developer ID 签名、经 Apple 公证、用户双击装不被 Gatekeeper 拦截，且 Jamf Pro / Kandji / Intune 都接受下发的 `.pkg`。

需要配置的 7 个 secret（都用 `gh secret set NAME --repo krislavten/cowork-mdm` 配置）：

| Secret | 内容 | 获取方式 |
|---|---|---|
| `MACOS_APP_CERT_P12` | Developer ID **Application** 证书，base64 编码的 `.p12` | Keychain Access → 右键证书 → 导出 → 设密码。然后 `base64 -i cert.p12 \| pbcopy`。 |
| `MACOS_APP_CERT_PASSWORD` | 上一步导出时设的密码 | — |
| `MACOS_INSTALLER_CERT_P12` | Developer ID **Installer** 证书，base64 编码的 `.p12`（和 Application 证书是两张独立证书，专门签 `.pkg` 外壳） | 同上。 |
| `MACOS_INSTALLER_CERT_PASSWORD` | installer `.p12` 的密码 | — |
| `APPLE_API_KEY_P8` | App Store Connect API key 的内容（`.p8` 文件的原始文本） | [App Store Connect → Users and Access → Integrations → App Store Connect API](https://appstoreconnect.apple.com/access/api)。创建 key 时选 **Developer** 角色。只能下载一次。 |
| `APPLE_API_KEY_ID` | Key 名字旁边那 10 个字符的 Key ID | 同上页面。 |
| `APPLE_API_ISSUER_ID` | 页面顶部的 Issuer UUID | 同上页面（每 team 一个）。 |

配好后推一个 patch tag，在 Actions 里看 `macos-pkg` job —— 应该看到 `build-pkg: signing binary with Developer ID Application (...)` 和 `build-pkg: notarized + stapled`，而不是 `skipping`。

证书 P12 都没配置时，跳过签名路径，产出 unsigned pkg。如果 P12 配了但密码错或缺失，keychain 导入会在任何 pkg 产出之前就失败 —— Release 里只会有 tar.gz + brew，没有 pkg。`continue-on-error: true` 保证在这种情况下 workflow 不会变红；修法是把密码配对，重推 tag。
